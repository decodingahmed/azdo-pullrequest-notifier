...
resource email_communications_service 'Microsoft.Communication/emailServices@2023-04-01' = {
  location: 'global'
  name: '${resource_prefix}-prnotifier-ecs'
  properties: {
    dataLocation: 'UK'
  }
}
...
resource communications_service 'Microsoft.Communication/CommunicationServices@2023-04-01' = {
  location: 'global'
  name: '${resource_prefix}-prnotifier-cs'
  properties: {
    dataLocation: 'UK'
    linkedDomains: [
...
resource keyvault 'Microsoft.KeyVault/vaults@2023-07-01' = {
  location: region
  name: '${resource_prefix}-prnotifier-kv'
  properties: {
    accessPolicies: []
...
var communications_service_connection_name = '${resource_prefix}-prnotifier-ecs-connection'
...
var keyvault_connection_name = '${resource_prefix}-prnotifier-kv-connection'
...
resource logic_app 'Microsoft.Logic/workflows@2019-05-01' = {
  ...
  location: region
  name: '${resource_prefix}-prnotifier-la'
  properties: {
    definition: {
      ...
      actions: {
        ...
        Get_All_Open_PRs: {
          ...
          inputs: {
            ...
            method: 'GET'
            uri: '@{variables(\'azdo_project_url\')}/_apis/git/pullrequests?api-version=6.0'
          }
        ...
        Init_Watched_Repositories: {
          inputs: {
            variables: [
              {
                name: 'azdo_repository_names'
                type: 'array'
                value: azdo_repository_names
              }
            ]
          }
          runAfter: {
        ...
        Init_Project_URL: {
          inputs: {
            variables: [
              {
                name: 'azdo_project_url'
                type: 'string'
                value: azdo_project_url
              }
            ]
          }
          runAfter: {}
